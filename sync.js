...
async function updateValidatorCore() {
  ...
  console.log('Elements found - commission:', !!comEl, 'power:', !!powerEl);
  console.log('Tokens:', tokens, 'Width:', window.innerWidth);

  if (comEl) comEl.textContent = commission.toFixed(1) + "%";
  if (powerEl) {
    // РАДИКАЛЬНЕ очищення - спочатку повністю очищаємо
    powerEl.innerHTML = '';
    
    // ОСТАТОЧНЕ виправлення - працюємо безпосередньо з div
    if (window.innerWidth <= 768) {
      // На мобільних: ТІЛЬКИ "10.617 M"
      const millions = (tokens / 1_000_000).toFixed(3);
      powerEl.textContent = millions + " M";
      console.log('Mobile format applied:', millions + " M");
    } else {
      // На десктопі: повне число "10,617,088"
      const formatted = tokens.toLocaleString("en-US");
      powerEl.textContent = formatted;
      console.log('Desktop format applied:', formatted);
    }
    console.log('Final content:', powerEl.textContent);
  } else {
    console.log('ERROR: Element delegatedAmountContainer not found');
  }
}
