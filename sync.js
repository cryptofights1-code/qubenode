...
async function updateValidatorCore() {
  ...
  // ПРОСТА МАТЕМАТИКА: токени вже в TICS (поділені на 1_000_000)
  ...
  if (powerEl) {
    ...
    // ПРОСТО ФОРМАТУЄМО: 10713591 -> "10,713 M"
    let formatted;
    if (tokens >= 1_000_000) {
      // Приклад: 10713591 / 1000 = 10713.591 -> Math.floor = 10713 -> "10,713 M"
      const thousands = Math.floor(tokens / 1000);
      formatted = thousands.toLocaleString("en-US") + " M";
    } else if (tokens >= 1_000) {
      const thousands = Math.floor(tokens / 1000);
      formatted = thousands.toLocaleString("en-US") + " K";
    } else {
    ...
    console.log('✅ Amount:', formatted, '(raw TICS:', tokens.toLocaleString(), ')');
  }
}
