// sync.js — фінальна робоча версія для сайту QubeNode
...
// Універсальна функція для запитів із ключем
...
async function updateBlockHeight() {
  ...
  if (data?.result?.sync_info?.latest_block_height) {
    document.getElementById('currentBlock').textContent =
      parseInt(data.result.sync_info.latest_block_height).toLocaleString('uk-UA');
  }
}

// Дані про валідатора
async function updateValidatorInfo() {
  const validator = await fetchData(`${RPC_BASE}/cosmos/staking/v1beta1/validators/${VALIDATOR_ADDR}`);
  ...
  const delegations = await fetchData(`${RPC_BASE}/cosmos/staking/v1beta1/validators/${VALIDATOR_ADDR}/delegations`);
...
async function updateTicsPrice() {
  const data = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=tics&vs_currencies=usd&include_24hr_change=true');
  if (data?.tics) {
