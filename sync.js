...
async function updateValidatorCore() {
  ...
  if (powerEl) {
    ...
    // Форматуємо в мільйони (M)
    let formatted;
    if (tokens >= 1_000_000) {
      // Якщо більше мільйона, показуємо в форматі "10,712 M"
      const millions = tokens / 1_000_000;
      formatted = millions.toLocaleString("en-US", {
      ...
      formatted = thousands.toLocaleString("en-US", {
        minimumFractionDigits: 0,
        maximumFractionDigits: 3
      }) + " K";
    } else {
...
function initBlockAnimation() {
  ...
  if (isMobile) {
    // На мобільних: ширина екрану мінус padding контейнера (20px з кожного боку)
