...
function initBlockAnimation() {
  ...
  const blockWidth = 6; // ширина палички
  const gapWidth = 8; // проміжок між паличками
  const totalBlockSpace = blockWidth + gapWidth; // 14px на одну паличку
  
  // Кількість паличок = ширина контейнера / простір на паличку
  const blocksCount = Math.floor(containerWidth / totalBlockSpace);
  
  console.log(`📊 Container width: ${containerWidth}px, creating ${blocksCount} blocks`);
  ...
  console.log(`✅ Block animation initialized with ${blocksCount} blocks (full container)`);
}
