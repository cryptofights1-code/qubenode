...
function initBlockAnimation() {
  ...
  if (isMobile) {
    // На мобільних: беремо ширину контейнера (не всього екрану!)
    // Це дозволить паличкам залишатися в межах видимості
  ...
  const blockWidth = isMobile ? 6 : 6;   // Однакова ширина
  const gapWidth = isMobile ? 3 : 8;     // Менший gap на мобільних
  ...
  // ОБМЕЖЕННЯ для мобільних: максимум 35 паличок для малих екранів
  if (isMobile) {
    const maxBlocks = window.innerWidth < 375 ? 30 : 35; // Для дуже малих екранів (iPhone SE) - 30
    blocksCount = Math.min(blocksCount, maxBlocks);
  }
