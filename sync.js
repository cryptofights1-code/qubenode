...
async function updateValidatorCore() {
  ...
  // Приклад: "10758095273067618117969514" 
  // 1 TICS = 1,000,000 uTICS (6 нулів)
  // 1 Million TICS = 1,000,000,000,000 uTICS (12 нулів)
  const tokensString = v.tokens.toString();
  
  // Використовуємо BigInt для точних обчислень великих чисел
  let millions;
  
  try {
    const tokensBigInt = BigInt(tokensString);
    const divisor = BigInt(1000000000000); // 12 нулів (uTICS -> Millions)
    const millionsBigInt = tokensBigInt / divisor;
    millions = Number(millionsBigInt);
  } catch (e) {
