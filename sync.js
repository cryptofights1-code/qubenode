...
async function updateValidatorCore() {
  ...
  if (powerEl) {
    ...
    // ПРОСТА ЛОГІКА - тільки мільйони та тисячі
    let formatted;
    if (tokens >= 1_000_000) {
      // Мільйони: ділимо на 1000000 і округлюємо до тисяч
      const millions = Math.floor(tokens / 1000); // Отримуємо тисячі
      const intPart = Math.floor(millions / 1000); // Цілі мільйони
      const decPart = millions % 1000; // Залишок в тисячах
      
      formatted = intPart.toLocaleString("en-US") + "," + decPart.toString().padStart(3, '0') + " M";
    } else if (tokens >= 1_000) {
      // Тисячі
      const intPart = Math.floor(tokens / 1000);
      const decPart = Math.floor((tokens % 1000));
      
      formatted = intPart.toLocaleString("en-US") + "," + decPart.toString().padStart(3, '0') + " K";
    } else {
      // Просто число
