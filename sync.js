...
async function updateValidatorCore() {
  ...
  // Приклад: "10711140153980156" або може бути з наукової нотації
  const tokensString = v.tokens.toString();
  
  // Якщо число в науковій нотації - parseFloat зламається
  // Працюємо зі СТРОКОЮ і видаляємо останні 12 цифр (6 нулів uTICS + 6 нулів millions)
  let millions;
  
  if (tokensString.length > 12) {
    // Відрізаємо останні 12 цифр
    const withoutLast12 = tokensString.slice(0, -12);
    millions = parseInt(withoutLast12);
  } else {
    // Якщо число мале
    millions = 0;
  }
  
  console.log('🔍 DEBUG: tokensString =', tokensString, '| Millions =', millions);
