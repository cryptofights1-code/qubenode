...
async function updateValidatorCore() {
  ...
  const tokens = parseInt(parseFloat(v.tokens) / 1_000_000);
  ...
  if (powerEl) {
    ...
    // Форматуємо в мільйони (M) - ТІЛЬКИ ЦІЛІ ЧИСЛА
    let formatted;
    if (tokens >= 1_000_000) {
      // Ділимо на мільйон та округлюємо до 3 знаків після коми
      const millions = Math.round((tokens / 1_000_000) * 1000) / 1000;
      
      // Форматуємо: розділяємо цілу та дробову частини
      const parts = millions.toFixed(3).split('.');
      const integerPart = parseInt(parts[0]).toLocaleString("en-US");
      const decimalPart = parts[1];
      
      // Видаляємо нулі в кінці дробової частини
      const trimmedDecimal = decimalPart.replace(/0+$/, '');
      
      if (trimmedDecimal) {
        formatted = integerPart + "." + trimmedDecimal + " M";
      } else {
        formatted = integerPart + " M";
      }
    } else if (tokens >= 1_000) {
      // Якщо більше тисячі, показуємо в форматі "10.712 K"
      const thousands = Math.round((tokens / 1_000) * 1000) / 1000;
      const parts = thousands.toFixed(3).split('.');
      const integerPart = parseInt(parts[0]).toLocaleString("en-US");
      const decimalPart = parts[1];
      const trimmedDecimal = decimalPart.replace(/0+$/, '');
      
      if (trimmedDecimal) {
        formatted = integerPart + "." + trimmedDecimal + " K";
      } else {
        formatted = integerPart + " K";
      }
    } else {
      // Інакше просто число
      formatted = tokens.toLocaleString("en-US");
    }
    
    const textNode = document.createTextNode(formatted);
    powerEl.appendChild(textNode);
    
    console.log('Amount updated:', formatted, '(raw:', tokens, ')');
  }
}
...
function initBlockAnimation() {
  ...
  if (isMobile) {
    // На мобільних: ширина екрану мінус padding контейнера (20px з кожного боку) + додатковий запас
  ...
  const blockWidth = isMobile ? 5 : 6;
  const gapWidth = isMobile ? 4 : 8; // Зменшив gap на мобільних з 6 до 4
