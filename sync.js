...
// --- UPTIME через slashing API (swagger.qubetics.com) ---
const VALCONS_ADDR = "qubeticsvalcons1dlmj5pzg3fv54nrtejnfxmrj08d7qs09xjp2eu";
const SLASHING_API = "https://swagger.qubetics.com/cosmos/slashing/v1beta1";
...
async function updateUptime() {
  ...
  try {
    const [infoRes, paramsRes] = await Promise.all([
      fetch(`${SLASHING_API}/signing_infos/${VALCONS_ADDR}`),
      fetch(`${SLASHING_API}/params`)
    ]);

    if (!infoRes.ok || !paramsRes.ok) {
      console.error("Uptime HTTP error", infoRes.status, paramsRes.status);
      el.textContent = "--";
      return;
    }
    ...
    const missedStr =
      info?.val_signing_info?.missed_blocks_counter ??
      info?.val_signing_info?.missed_blocks ??
      "0";

    const windowStr =
      params?.params?.signed_blocks_window ??
      params?.params?.signed_blocks_window_size ??
      "100000";

    const missed = parseInt(missedStr, 10);
    const windowSize = parseInt(windowStr, 10) || 100000;
