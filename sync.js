// === QubeNode Live Sync Script v2.6 ===
// Includes: validator info, delegators, inflation, uptime, block time, signed blocks, TICS price from MEXC
// Fixed: Removed duplicate VALCONS_ADDR declaration
...
// === SIGNED BLOCKS (blocks proposed by QubeNode validator) ===
async function updateSignedBlocks() {
  const el = document.getElementById("signedBlocks");
  if (!el) return;

  try {
    // Використовуємо Cosmos Slashing API для точного підрахунку
    // Формула: index_offset - missed_blocks = кількість підписаних блоків
    const infoUrl = `${API_BASE}/cosmos/slashing/v1beta1/signing_infos?pagination.limit=1000`;
    ...
    const entry = Array.isArray(list)
      ? list.find(i => 
          i.address === VALCONS_ADDR || 
          i.cons_address === VALCONS_ADDR || 
          i.valcons_address === VALCONS_ADDR
        )
      : null;
    ...
    if (entry) {
      ...
      if (indexOffset > 0) {
        // Це кількість блоків які валідатор підписав (не пропустив)
        const signedBlocks = Math.max(0, indexOffset - missedBlocks);
        el.textContent = signedBlocks.toLocaleString("en-US");
        console.log('✅ Signed blocks (Cosmos API):', signedBlocks);
        return;
      }
    }

    // Якщо Cosmos API не працює - показуємо placeholder
    el.textContent = "--";
    console.warn('⚠️ Signed blocks data unavailable');
...
async function updateTicsPrice() {
  ...
  try {
    ...
    if (data && data.lastPrice) {
      ...
      console.log(`✅ TICS price: $${price.toFixed(6)} (${changeText})`);
      return;
    }
