...
async function updateValidatorCore() {
  ...
  // v.tokens приходить у форматі uTICS (micro TICS)
  // Приклад з вашого скріншота: "10709816553980156" uTICS
  const uTokens = parseFloat(v.tokens);
  
  // КРОК 1: Конвертуємо в TICS: ділимо на 1_000_000
  // 10709816553980156 / 1000000 = 10709816553.980156 TICS
  const tics = uTokens / 1_000_000;
  
  // КРОК 2: Показуємо в МІЛЬЙОНАХ: ділимо на 1_000_000
  // 10709816553 / 1000000 = 10709.816553
  // Округляємо до цілого: 10709 або 10710
  const millions = Math.floor(tics / 1_000_000);
  
  console.log('🔍 DEBUG: uTICS =', uTokens, '| TICS =', tics.toFixed(2), '| Millions =', millions);
  ...
  if (powerEl) {
    ...
    // Форматуємо число з комами: 10709 -> "10,709"
    ...
    const formatted = result + " M";
    const textNode = document.createTextNode(formatted);
    powerEl.appendChild(textNode);
    
    console.log('✅ DELEGATED AMOUNT:', formatted, '| uTICS:', uTokens, '| TICS:', tics.toFixed(2), '| Millions:', millions);
  }
}
...
function addNewBlockVisual() {
  ...
  // Видаляємо підсвічування через 1200мс (щоб анімація повністю показалась)
  setTimeout(() => {
    block.classList.remove('fresh');
    console.log('⚪ .fresh class removed after 1200ms');
  }, 1200);
...
function initBlockAnimation() {
  ...
  if (isMobile) {
    ...
    // Віднімаємо більше щоб палички точно вмістилися
    containerWidth = window.innerWidth - 60; // Було 20, зменшив до 60
  } else {
    // На desktop: реальна ширина контейнера
    containerWidth = container.offsetWidth || 800;
  }
  
  const blockWidth = isMobile ? 8 : 6; // Збільшив з 5 до 8 щоб краще видно
  const gapWidth = isMobile ? 4 : 8;
