...
function updateDelegationModal() {
    ...
    const availableBalanceEl = document.querySelector('#availableBalance');
    const delegatedAmountEl = document.querySelector('#currentDelegation');
    const pendingRewardsEl = document.querySelector('#pendingRewards');
    
    if (walletAddressEl && window.userAddress) {
        walletAddressEl.textContent = window.userAddress.substring(0, 12) + '...' + window.userAddress.substring(window.userAddress.length - 8);
    }
    
    if (availableBalanceEl && typeof window.userBalance !== 'undefined') {
        availableBalanceEl.textContent = window.userBalance.toFixed(2) + ' TICS';
    }
    
    if (delegatedAmountEl && typeof window.delegatedAmount !== 'undefined') {
        delegatedAmountEl.textContent = window.delegatedAmount.toFixed(2) + ' TICS';
    }
    
    if (pendingRewardsEl && typeof window.rewardsAmount !== 'undefined') {
        pendingRewardsEl.textContent = window.rewardsAmount.toFixed(6) + ' TICS';
    }
...
async function broadcastTransaction(signedTx) {
    try {
        ...
        if (result.result && result.result.code === 0) {
            ...
            throw new Error(result.result?.log || result.error?.message || 'Broadcast failed');
        }
