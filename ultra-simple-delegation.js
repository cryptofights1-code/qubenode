...
async function broadcastTransaction(signedTx) {
    try {
        ...
        // Create the transaction payload for REST API
        // REST API expects the signed transaction in a specific format
        const txBody = {
            tx: signedTx,
            mode: 'BROADCAST_MODE_SYNC'
        };
        
        console.log('📤 Broadcasting via REST API...');
        ...
        if (result.tx_response) {
            ...
            if (result.tx_response.code === 0 || !result.tx_response.code) {
                return {
                    success: true,
