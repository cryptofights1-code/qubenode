...
// Update wallet data - USE KEPLR DIRECTLY
window.updateWalletData = async function() {
    console.log('🔄 Updating wallet data via Keplr...');
    ...
    try {
        ...
        // Get balance directly from Keplr (most reliable!)
        console.log('💰 Getting balance from Keplr...');
        let balanceInTics = 0;
        
        try {
            const offlineSigner = window.keplr.getOfflineSigner(chainId);
            const client = await window.keplr.getStargateClient(chainId);
            
            // Get all balances
            const allBalances = await client.getAllBalances(window.userAddress);
            console.log('💰 All balances from Keplr:', allBalances);
            
            const ticsBalance = allBalances.find(b => b.denom === 'utics');
            balanceInTics = ticsBalance ? parseFloat(ticsBalance.amount) / Math.pow(10, 18) : 0;
            
            console.log('💰 Balance from Keplr:', balanceInTics, 'TICS');
            
        } catch (keplrError) {
            console.log('⚠️ Keplr balance fetch failed, trying API...');
        ...
        // Get delegations from API (Keplr doesn't have this)
        ...
        // Try different fields for rewards
        let rewardsAmount = 0;
        
        if (rewardsData.rewards && rewardsData.rewards.length > 0) {
            const reward = rewardsData.rewards.find(r => r.denom === 'utics');
            if (reward) {
                rewardsAmount = parseFloat(reward.amount) / Math.pow(10, 18);
            }
        }
        
        // If still 0, try 'total' field
        if (rewardsAmount === 0 && rewardsData.total && rewardsData.total.length > 0) {
            const totalReward = rewardsData.total.find(r => r.denom === 'utics');
            if (totalReward) {
                rewardsAmount = parseFloat(totalReward.amount) / Math.pow(10, 18);
                console.log('🎁 Using total field for rewards');
            }
        }
        ...
        console.log('   Balance:', window.userBalance, 'TICS');
        console.log('   Delegated:', window.delegatedAmount, 'TICS');
        console.log('   Rewards:', window.rewardsAmount, 'TICS');
...
async function broadcastTransaction(signedTx) {
    try {
        ...
        const txBody = {
            ...
            mode: 'BROADCAST_MODE_SYNC'  // Correct enum value
        };
...
async function createAndSignTx(msgs, memo) {
    const chainId = 'qubetics_9030-1';
    const walletType = window.currentWallet || 'keplr';
    ...
    let signed;
    if (walletType === 'cosmostation') {
        signed = await window.cosmostation.providers.keplr.signAmino(chainId, account.address, signDoc);
    } else {
        signed = await window.keplr.signAmino(chainId, account.address, signDoc);
    }
