// === QubeNode Wallet Integration v2.0.0 ===
...
console.log('🔐 QubeNode Wallet Integration v2.0.0 LOADED');

// Qubetics Network Configuration
const QUBETICS_CHAIN_ID = "qubetics-1";
...
async function suggestQubeticsChain(wallet) {
  if (wallet === 'keplr') {
    ...
    try {
      ...
      console.log('✅ Qubetics chain already added to Keplr');
      return true;
    } catch (error) {
      try {
        await window.keplr.experimentalSuggestChain(CHAIN_CONFIG);
        console.log('✅ Qubetics chain suggested to Keplr');
        await window.keplr.enable(QUBETICS_CHAIN_ID);
        return true;
      } catch (e) {
        console.error('❌ Failed to suggest chain to Keplr:', e);
        throw new Error("Не вдалося додати мережу Qubetics до Keplr");
      }
    }
    ...
    try {
      ...
      console.log('✅ Qubetics chain already added to Cosmostation');
      return true;
    } catch (error) {
      try {
        const provider = window.cosmostation.providers.keplr;
        await provider.experimentalSuggestChain(CHAIN_CONFIG);
        console.log('✅ Qubetics chain suggested to Cosmostation');
        await provider.enable(QUBETICS_CHAIN_ID);
        return true;
      } catch (e) {
        console.error('❌ Failed to suggest chain to Cosmostation:', e);
        throw new Error("Не вдалося додати мережу Qubetics до Cosmostation");
      }
    }
  }
}
...
function updateHeaderAfterConnection() {
  ...
  const shortAddress = userAddress.substring(0, 12) + '...' + userAddress.substring(userAddress.length - 4);
...
async function updateWalletData() {
  ...
  try {
    ...
    if (ticsBalance) {
      ...
      userBalance = 0;
      console.log('⚠️ No TICS balance found');
    }
...
async function delegateTokens() {
  ...
  if (amount < 1) {
    alert('Мінімальна сума для делегування: 1 TICS');
    return;
  }
